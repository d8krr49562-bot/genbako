...
const APP_VERSION = "Ver 3.6"; // ファイルを渡すたびに番号を上げていく(トムが更新を確認できるように)
...
const emptyJob = (taxEnabled = false, taxRate = 10) => ({
...
export default function InvoiceApp() {
  ...
  const [alwaysTaxEnabled, setAlwaysTaxEnabled] = useState(false); // ONの時、新規入力は毎回自動で税金がかかる
  const [alwaysTaxRate, setAlwaysTaxRate] = useState(10);
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        setAlwaysTaxEnabled(!!data.alwaysTaxEnabled);
        setAlwaysTaxRate(data.alwaysTaxRate || 10);
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    try {
      const payload = JSON.stringify({ entries, companies, sitePresets, namePresets, ninkuPresets, overtimeRatePresets, comboPresets, routes, highwayPresets, profile, paymentStatus, pdfLayout, alwaysTaxEnabled, alwaysTaxRate });
  ...
  }, [entries, companies, sitePresets, namePresets, ninkuPresets, overtimeRatePresets, comboPresets, routes, highwayPresets, profile, paymentStatus, pdfLayout, alwaysTaxEnabled, alwaysTaxRate, loading]);
  ...
  if (view === "dayJobs" && selectedDate) {
    return (
      <DayJobsView
        ...
        onUpdateExpenses={(idx, expenses) => {
          const jobs = entries[selectedDate] || [];
          if (!jobs[idx]) return;
          updateJobInDay(selectedDate, idx, { ...jobs[idx], expenses });
        }}
  ...
  if (view === "jobEdit" && selectedDate) {
    const jobs = entries[selectedDate] || [];
    const initial = editingIndex !== null ? jobs[editingIndex] : (prefillJob || emptyJob(alwaysTaxEnabled, alwaysTaxRate));
    return (
      <JobDetail
        dateKey={selectedDate}
        job={initial}
  ...
  if (view === "settings") {
    return (
      <SettingsView
        ...
        alwaysTaxEnabled={alwaysTaxEnabled}
        setAlwaysTaxEnabled={setAlwaysTaxEnabled}
        alwaysTaxRate={alwaysTaxRate}
        setAlwaysTaxRate={setAlwaysTaxRate}
        onBack={() => setView("calendar")}
      />
    );
  }
...
// ------------------- その日の仕事一覧画面 -------------------
function DayJobsView({ dateKey, jobs, comboPresets, sitePresets, companies, ninkuPresets, onBack, onAddQuick, onCreateCombo, onOpenNew, onOpenEdit, onRemove, onRemoveCombo, onSetSite, onUpdateExpenses, setSitePresets }) {
  ...
  const [showPickJobForExpense, setShowPickJobForExpense] = useState(false); // 諸経費を編集する仕事を選ぶ画面
  const [expenseQuickIdx, setExpenseQuickIdx] = useState(null); // 諸経費をクイック編集中の仕事のインデックス
    ...
    <div style={{ minHeight: "100vh", background: "#1C1F26", fontFamily: "'Zen Kaku Gothic New','Hiragino Sans',sans-serif" }}>
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "18px 16px 60px" }}>
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          {jobs.length > 0 && (
            <button
              onClick={() => (jobs.length === 1 ? setExpenseQuickIdx(0) : setShowPickJobForExpense(true))}
              style={{
                marginTop: 8, width: "100%", background: "#242832", border: "1px solid #F5A623", borderRadius: 10,
                color: "#F5A623", fontSize: 12, fontWeight: 700, cursor: "pointer", padding: "9px",
                display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
              }}
            >
              <Zap size={13} /> 諸経費を編集する
            </button>
          )}
        </div>
      ...
      {showPickJobForExpense && (
        <Modal onClose={() => setShowPickJobForExpense(false)}>
          <div style={{ color: "#fff", fontSize: 15, fontWeight: 700, marginBottom: 10 }}>どの仕事の諸経費を編集しますか？</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {jobs.map((j, idx) => (
              <button
                key={idx}
                onClick={() => { setShowPickJobForExpense(false); setExpenseQuickIdx(idx); }}
                style={{
                  width: "100%", textAlign: "left", background: "#242832", border: "1px solid #333846", borderRadius: 10,
                  padding: "10px 12px", color: "#fff", fontSize: 13, cursor: "pointer",
                }}
      ...
      {expenseQuickIdx !== null && jobs[expenseQuickIdx] && (
        <QuickExpenseModal
          job={jobs[expenseQuickIdx]}
          onClose={() => setExpenseQuickIdx(null)}
          onSave={(expenses) => { onUpdateExpenses(expenseQuickIdx, expenses); setExpenseQuickIdx(null); }}
        />
      )}
...
function QuickExpenseModal({ job, onClose, onSave }) {
  const [expenses, setExpenses] = useState(job.expenses && job.expenses.length ? job.expenses : []);
  const [confirmDeleteId, setConfirmDeleteId] = useState(null);

  const addQuick = (label) => setExpenses((prev) => [...prev, { id: Date.now() + Math.random(), label, amount: 0 }]);
  const updateLabel = (id, label) => setExpenses((prev) => prev.map((e) => (e.id === id ? { ...e, label } : e)));
  const updateAmount = (id, amountStr) => setExpenses((prev) => prev.map((e) => (e.id === id ? { ...e, amount: Number(amountStr.replace(/[^0-9]/g, "") || 0) } : e)));
  const removeExpense = (id) => setExpenses((prev) => prev.filter((e) => e.id !== id));

  return (
    <Modal onClose={onClose}>
      <div style={{ color: "#fff", fontSize: 15, fontWeight: 700, marginBottom: 4 }}>諸経費を編集</div>
      <div style={{ color: "#8A8F9C", fontSize: 11, marginBottom: 10 }}>
        {job.company || "未設定"}{job.site ? `　${job.site}` : ""}
      </div>

      <ChipRow>
        {EXPENSE_LABEL_PRESETS.map((label) => (
          <Chip key={label} onClick={() => addQuick(label)}>{label}</Chip>
        ))}
        <Chip onClick={() => addQuick("")} isAdd><Plus size={14} /> その他</Chip>
      </ChipRow>
        ...
        <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
          {expenses.map((e) => (
            <div key={e.id} style={{ display: "flex", alignItems: "center", gap: 6, background: "#242832", border: "1px solid #333846", borderRadius: 10, padding: "8px 10px", flexWrap: "wrap" }}>
              ...
              <button onClick={() => setConfirmDeleteId(e.id)} style={{ background: "none", border: "none", cursor: "pointer", padding: 2 }}>
                <Trash2 size={14} color="#8A8F9C" />
              </button>
            </div>
          ))}
          <div style={{ color: "#8FD19E", fontSize: 12, fontWeight: 700, textAlign: "right" }}>小計 {yen(expensesTotal(expenses))}</div>
        </div>
      )}

      <button onClick={() => onSave(expenses)} style={modalBtnStyle}>この内容で保存する</button>

      {confirmDeleteId !== null && (
        <Modal onClose={() => setConfirmDeleteId(null)}>
          <div style={{ color: "#fff", fontSize: 15, fontWeight: 700, marginBottom: 6 }}>削除しますか？</div>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => setConfirmDeleteId(null)} style={{ flex: 1, background: "#242832", border: "1px solid #444A58", borderRadius: 10, padding: "11px", color: "#C7CBD4", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>キャンセル</button>
            <button
              onClick={() => { removeExpense(confirmDeleteId); setConfirmDeleteId(null); }}
              style={{ flex: 1, background: "#E85D5D", border: "none", borderRadius: 10, padding: "11px", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer" }}
            >
              削除する
            </button>
...
function JobDetail({ dateKey, job, onSave, onBack, companies, setCompanies, sitePresets, setSitePresets, namePresets, setNamePresets, ninkuPresets, setNinkuPresets, overtimeRatePresets, setOvertimeRatePresets, onSaveCombo, comboPresets, routes, setRoutes, highwayPresets, setHighwayPresets, entries }) {
  ...
  const addPerson = () => {
    setExtraPeople((prev) => [...prev, {
      ...
      taxEnabled, taxRateStr, // 本体で今設定してる税金の状態をそのまま引き継ぐ
    }]);
  };
...
// ------------------- 発行者名・振込先の設定画面 -------------------
// ------------------- 設定画面 -------------------
function SettingsView({ pdfLayout, setPdfLayout, alwaysTaxEnabled, setAlwaysTaxEnabled, alwaysTaxRate, setAlwaysTaxRate, onBack }) {
  const [rateStr, setRateStr] = useState(String(alwaysTaxRate || 10));
  return (
    <div style={{ minHeight: "100vh", background: "#1C1F26", fontFamily: "'Zen Kaku Gothic New','Hiragino Sans',sans-serif" }}>
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "18px 16px 60px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18 }}>
        ...
        <Section label="全てに税金を反映させる">
          <div style={{ color: "#8A8F9C", fontSize: 11, marginBottom: 10 }}>
            ONにすると、新しく入力する仕事すべてに、毎回ボタンを押さなくても自動で税金がかかるようになります（人工・残業代・諸経費が対象）。すでに入力済みの仕事には影響しません。
          </div>
          <Chip active={alwaysTaxEnabled} onClick={() => setAlwaysTaxEnabled((v) => !v)}>常に税金を適用する{alwaysTaxEnabled ? "✓" : ""}</Chip>
          {alwaysTaxEnabled && (
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 10 }}>
              <input
                type="text" inputMode="numeric" value={rateStr}
                onChange={(e) => {
                  const v = e.target.value.replace(/[^0-9]/g, "");
                  setRateStr(v);
                  setAlwaysTaxRate(Number(v || 0));
                }}
    ...
    <div style={{ minHeight: "100vh", background: "#3A3D45", overflowX: "auto" }}>
      ...
      <style>{`
        @media print {
          ...
          .pdf-page-break-after { display: block; page-break-after: always; break-after: page; }
