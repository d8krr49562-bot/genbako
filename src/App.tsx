...
// 諸経費のよく使う項目名プリセット
const EXPENSE_LABEL_PRESETS = ["交通費", "燃料費", "高速代", "宿泊費"];
...
function JobDetail({ dateKey, job, onSave, onBack, companies, setCompanies, sitePresets, setSitePresets, ninkuPresets, setNinkuPresets, onSaveCombo, comboPresets, routes, setRoutes }) {
  ...
  const registerRoute = () => {
    if (!newRouteKm) return; // 距離だけは必須、出発地・到着地・住所は任意
    const r = {
      id: Date.now(),
      from: newRouteFrom.trim() || "出発地",
      to: newRouteTo.trim() || "到着地",
    ...
    <div style={{ minHeight: "100vh", background: "#1C1F26", fontFamily: "'Zen Kaku Gothic New','Hiragino Sans',sans-serif" }}>
      ...
      {showAddRoute && (
        <Modal onClose={() => setShowAddRoute(false)}>
          ...
          <div style={{ color: "#8A8F9C", fontSize: 11, marginBottom: 6 }}>出発地（呼び名・任意）</div>
          ...
          <div style={{ color: "#8A8F9C", fontSize: 11, marginTop: 16, marginBottom: 6 }}>到着地（呼び名・任意）</div>
          ...
          <div style={{ color: "#8A8F9C", fontSize: 11, marginTop: 16, marginBottom: 6 }}>距離(km)</div>
