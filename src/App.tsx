...
const emptyJob = () => ({
  company: "", site: "", ninku: 0, overtimeMode: "time", overtimeMin: 0,
...
export default function InvoiceApp() {
  ...
  const [profile, setProfile] = useState({ issuerName: "", bankInfo: "", closingDay: "末日" }); // 発行者名・振込先・締め日(共通設定)
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        setProfile(data.profile || { issuerName: "", bankInfo: "", closingDay: "末日" });
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    try {
      const payload = JSON.stringify({ entries, companies, sitePresets, ninkuPresets, comboPresets, routes, profile });
  ...
  }, [entries, companies, sitePresets, ninkuPresets, comboPresets, routes, profile, loading]);
    ...
    <div style={{ minHeight: "100vh", background: "#1C1F26", fontFamily: "'Zen Kaku Gothic New','Hiragino Sans',sans-serif" }}>
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "20px 16px 40px" }}>
        ...
        <button
          ...
          style={{ marginTop: 10, width: "100%", background: "#242832", border: "1px dashed #F5A623", borderRadius: 12, padding: "11px", color: "#F5A623", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}
        >
              ...
              <div key={idx} style={{ background: "#242832", border: "1px solid #333846", borderRadius: 12, padding: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <button onClick={() => onOpenEdit(idx)} style={{ background: "none", border: "none", textAlign: "left", flex: 1, cursor: "pointer", padding: 0 }}>
                    <div style={{ color: "#fff", fontSize: 14, fontWeight: 700 }}>{j.company || "未設定"}{j.site ? `　${j.site}` : ""}</div>
...
// ------------------- 1件分の入力/編集画面 -------------------
function JobDetail({ dateKey, job, onSave, onBack, companies, setCompanies, sitePresets, setSitePresets, ninkuPresets, setNinkuPresets, onSaveCombo, comboPresets, routes, setRoutes }) {
  ...
  const currentJob = () => ({ company, site, ninku: Number(ninku), overtimeMode, overtimeMin, overtimeRatePerHour, overtimeAmount, expenses, memo: "" });
  ...
  const save = () => onSave({ company, site, ninku: Number(ninku), overtimeMode, overtimeMin, overtimeRatePerHour, overtimeAmount, expenses, memo });
  ...
  const [newRouteFromAddress, setNewRouteFromAddress] = useState("");
  const [newRouteToAddress, setNewRouteToAddress] = useState("");
  ...
  const registerRoute = () => {
    ...
    const r = {
      id: Date.now(), from: newRouteFrom.trim(), to: newRouteTo.trim(),
      fromAddress: newRouteFromAddress.trim(), toAddress: newRouteToAddress.trim(),
      km: Number(newRouteKm),
    };
    setRoutes((prev) => [...prev, r]);
    selectRoute(r);
    setShowAddRoute(false);
    setNewRouteFrom(""); setNewRouteTo(""); setNewRouteFromAddress(""); setNewRouteToAddress(""); setNewRouteKm("");
  };

  // 距離だけ入力するモード(ルート登録なしでその場だけ計算)
  const openKmOnlyCalc = () => {
    setRouteFrom("");
    setRouteTo("");
    setRouteKmStr("");
    setUseHighway(false);
    ...
    <div style={{ minHeight: "100vh", background: "#1C1F26", fontFamily: "'Zen Kaku Gothic New','Hiragino Sans',sans-serif" }}>
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "18px 16px 60px" }}>
        ...
        <Section label="交通費・諸経費">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <div style={{ color: "#8A8F9C", fontSize: 11 }}>ルートを選んで燃料費・高速代を自動計算</div>
            {routes.length > 0 && <EditToggleButton editing={editingRoutes} onClick={() => setEditingRoutes((v) => !v)} />}
          </div>
          <ChipRow>
            {routes.map((r) => (
              <ChipWithDelete key={r.id} editing={editingRoutes} onDelete={() => requestDelete("route", r.id, `${r.from}→${r.to}`)}>
                <Chip onClick={() => { if (editingRoutes) return; selectRoute(r); setShowRouteCalc(true); }}>
                  {r.from}→{r.to}（{r.km}km）
                </Chip>
              </ChipWithDelete>
            ))}
            {!editingRoutes && <Chip onClick={() => setShowAddRoute(true)} isAdd><Plus size={14} /> ルート登録</Chip>}
            {!editingRoutes && <Chip onClick={openKmOnlyCalc}>距離だけ入力する</Chip>}
          </ChipRow>
            ...
            <div style={{ marginTop: 10, background: "#242832", border: "1px solid #333846", borderRadius: 12, padding: 12 }}>
              {routeFrom || routeTo ? (
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                  <input value={routeFrom} onChange={(e) => setRouteFrom(e.target.value)} placeholder="出発地（任意）" style={{ ...inputStyle, flex: 1, boxSizing: "border-box" }} />
                  <input value={routeTo} onChange={(e) => setRouteTo(e.target.value)} placeholder="到着地（任意）" style={{ ...inputStyle, flex: 1, boxSizing: "border-box" }} />
                </div>
              )}
              <div style={{ display: "flex", gap: 8 }}>
      ...
      {showAddRoute && (
        <Modal onClose={() => setShowAddRoute(false)}>
          ...
          <div style={{ color: "#8A8F9C", fontSize: 11, marginBottom: 6 }}>出発地（呼び名）</div>
          ...
          <div style={{ color: "#8A8F9C", fontSize: 11, marginTop: 10, marginBottom: 6 }}>出発地の住所（任意）</div>
          <input value={newRouteFromAddress} onChange={(e) => setNewRouteFromAddress(e.target.value)} placeholder="例: 東京都〇〇区〇〇1-2-3" style={{ ...inputStyle, width: "100%", boxSizing: "border-box" }} />

          <div style={{ color: "#8A8F9C", fontSize: 11, marginTop: 16, marginBottom: 6 }}>到着地（呼び名）</div>
          ...
          <div style={{ color: "#8A8F9C", fontSize: 11, marginTop: 10, marginBottom: 6 }}>到着地の住所（任意）</div>
          <input value={newRouteToAddress} onChange={(e) => setNewRouteToAddress(e.target.value)} placeholder="例: 東京都〇〇区〇〇4-5-6" style={{ ...inputStyle, width: "100%", boxSizing: "border-box" }} />

          <div style={{ color: "#8A8F9C", fontSize: 11, marginTop: 16, marginBottom: 6 }}>距離(km) ※Googleマップ等で調べて入力</div>
...
function InvoiceView({ year, month, entries, onBack, profile, setProfile, onOpenDate }) {
  const rows = useMemo(() => {
    ...
    Object.entries(entries)
      ...
      .forEach(([k, jobs]) => {
        (jobs || []).forEach((j) => {
          out.push({ date: k, day: Number(k.split("-")[2]), company: j.company || "-", site: j.site || "", ninku: j.ninku || 0, overtime: j.overtimeAmount || 0, transport: expensesTotal(j.expenses), expenses: j.expenses || [], total: jobTotal(j) });
        });
      });
    return out;
  }, [entries, year, month]);
  ...
  if (showPdfPreview) {
    const filteredRows = pdfCompanyFilter === "__all__" ? rows : rows.filter((r) => r.company === pdfCompanyFilter);
    const filteredTotal = filteredRows.reduce((s, r) => s + r.total, 0);
    return (
      <PdfPreview
        year={year} month={month} rows={filteredRows} grandTotal={filteredTotal}
...
function ProfileEditView({ profile, setProfile, onBack }) {
  ...
  const save = () => {
    setProfile({ issuerName, bankInfo, closingDay });
    setSaved(true);
...
function PdfPreview({ year, month, rows, grandTotal, companyLabel, profile, onBack }) {
  ...
  const [editableRows, setEditableRows] = useState(
    rows.map((r) => ({ ...r, dateLabel: `${month}月${r.day}日`, content: r.site || r.company || "" }))
  );
    ...
    <div style={{ minHeight: "100vh", background: "#3A3D45", overflowX: "auto" }}>
          ...
          <tbody>
            {editableRows.map((r, i) => (
              <tr key={i}>
                ...
                <td style={{ border: "1px solid #ccc", padding: 2 }}><input value={r.content} onChange={(e) => updateRow(i, "content", e.target.value)} style={printFieldStyle} /></td>
                ...
                <td style={{ border: "1px solid #ccc", padding: 2 }}>
                  <input value={r.transport} onChange={(e) => updateRow(i, "transport", e.target.value)} style={{ ...printFieldStyle, textAlign: "right" }} />
                  {r.expenses && r.expenses.length > 0 && (
                    <div style={{ fontSize: 8.5, color: "#666", textAlign: "right", padding: "0 4px", lineHeight: 1.4 }}>
                      {r.expenses.map((e, ei) => (
      ...
      <style>{`
        @media print {
          ...
          .pdf-paper { position: absolute; top: 0; left: 0; width: 100%; max-width: none !important; box-shadow: none !important; margin: 0 !important; }
