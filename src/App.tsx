...
// ------------------- 1件分の入力/編集画面 -------------------
function JobDetail({ dateKey, job, onSave, onBack, companies, setCompanies, sitePresets, setSitePresets, ninkuPresets, setNinkuPresets, onSaveCombo, comboPresets, routes, setRoutes, entries }) {
  const [company, setCompany] = useState(job.company || "");
  const [site, setSite] = useState(job.site || "");
  const [siteAddress, setSiteAddress] = useState(job.siteAddress || "");
  const [personName, setPersonName] = useState(job.personName || "");
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "18px 16px 60px" }}>
        ...
        {extraPeople.map((p, idx) => (
          <div key={p.id} style={{ marginTop: 18, background: "#20242E", border: "1px solid #F5A623", borderRadius: 14, padding: 14 }}>
            ...
            <div style={{ color: "#8A8F9C", fontSize: 11, marginBottom: 6 }}>名前（任意）</div>
            <ManualInput placeholder="例: 佐藤" value={p.name} onChange={(v) => updatePerson(p.id, { name: v })} />

            <div style={{ color: "#8A8F9C", fontSize: 11, margin: "12px 0 6px" }}>人工</div>
            <ChipRow>
              {ninkuPresets.map((v) => (
            ...
            <div style={{ color: "#8A8F9C", fontSize: 11, margin: "12px 0 6px" }}>残業代</div>
            <ChipRow>
            ...
            <div style={{ color: "#8A8F9C", fontSize: 11, margin: "14px 0 6px" }}>交通費</div>
            <ChipRow>
              {routes.filter((r) => r.label).map((r) => (
                    ...
                    <div style={{ marginTop: 8 }}>
                      ...
                      {t.useHighway && (
                        <div style={{ marginTop: 8 }}>
                          ...
                          <input value={t.highwayMemo || ""} onChange={(e) => updatePersonTransport(p.id, t.id, { highwayMemo: e.target.value })} placeholder="例: 〇〇IC〜〇〇IC" style={{ ...inputStyle, width: "100%", boxSizing: "border-box", marginTop: 8 }} />
                        </div>
                      )}
                    </div>
            ...
            <div style={{ color: "#8A8F9C", fontSize: 11, margin: "14px 0 6px" }}>諸経費</div>
            <ChipRow>
              {EXPENSE_LABEL_PRESETS.map((label) => (
            ...
            {p.expenses.length > 0 && (
              <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 6 }}>
                {p.expenses.map((e) => (
                  <div key={e.id} style={{ display: "flex", alignItems: "center", gap: 8, background: "#242832", border: "1px solid #333846", borderRadius: 10, padding: "8px 10px" }}>
                    <input value={e.label} onChange={(ev) => updatePersonExpense(p.id, e.id, { label: ev.target.value })} style={{ ...inputStyle, flex: 1, background: "transparent", border: "none", padding: "2px 0" }} />
                    <input type="text" inputMode="numeric" placeholder="0" value={e.amount === 0 ? "" : String(e.amount)} onChange={(ev) => updatePersonExpense(p.id, e.id, { amount: Number(ev.target.value.replace(/[^0-9]/g, "") || 0) })} style={{ ...inputStyle, width: 90, textAlign: "right" }} />
                    <span style={{ color: "#5A5F6B", fontSize: 11 }}>円</span>
                    <button onClick={() => removePersonExpense(p.id, e.id)} style={{ background: "none", border: "none", cursor: "pointer", padding: 2 }}>
                      <Trash2 size={13} color="#5A5F6B" />
                    </button>
                  </div>
                ))}
              </div>
    ...
    Object.entries(entries)
      ...
      .forEach(([k, jobs]) => {
        (jobs || []).forEach((j) => {
          ...
          const combinedTransports = [...(j.transports || []), ...extra.flatMap((p) => p.transports || [])];
          const combinedExpenses = [
            ...(j.expenses || []).map((e) => ({ ...e, label: j.personName ? `${j.personName}：${e.label}` : e.label })),
            ...extra.flatMap((p) => (p.expenses || []).map((e) => ({ ...e, label: p.name ? `${p.name}：${e.label}` : e.label }))),
          ];
          out.push({
        ...
        (jobs || []).forEach((j) => {
          ...
          (j.extraPeople || []).forEach((p) => {
            out.push({
              ...
              transport: transportsTotal(p.transports), transports: p.transports || [],
              miscExpense: expensesTotal(p.expenses), expenses: p.expenses || [],
          ...
          <tbody>
            {editableRows.map((r, i) => (
              <tr key={i}>
                ...
                <td style={{ border: "1px solid #ccc", padding: 2, wordBreak: "break-word" }}><input value={r.addressLabel} onChange={(e) => updateRow(i, "addressLabel", e.target.value)} style={printFieldStyle} /></td>
