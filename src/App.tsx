...
// 1件分の仕事の合計金額
const jobTotal = (j) => (j.ninku || 0) + (j.overtimeAmount || 0) + expensesTotal(j.expenses) + transportsTotal(j.transports);
// その日全体(複数件)の合計金額
...
const emptyJob = () => ({
  ...
  overtimeRatePerHour: 2500, overtimeAmount: 0, expenses: [], transports: [], memo: "",
});
  ...
  if (view === "jobEdit" && selectedDate) {
    ...
    return (
      <JobDetail
        ...
        onSaveAndAddPerson={(job) => {
          if (editingIndex !== null) updateJobInDay(selectedDate, editingIndex, job);
          else addJobToDay(selectedDate, job);
          // 会社・現場名だけ引き継いで、新しい1人分の入力を開く
          setPrefillJob({ ...emptyJob(), company: job.company, site: job.site });
              ...
              <div key={idx} style={{ background: "#242832", border: "1px solid #333846", borderRadius: 12, padding: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <button onClick={() => onOpenEdit(idx)} style={{ background: "none", border: "none", textAlign: "left", flex: 1, cursor: "pointer", padding: 0 }}>
                    ...
                    <div style={{ color: "#6B7280", fontSize: 11, marginTop: 2 }}>人工{yen(j.ninku)} ／ 残業{yen(j.overtimeAmount)} ／ 交通費{yen(transportsTotal(j.transports))} ／ 諸経費{yen(expensesTotal(j.expenses))}</div>
                  </button>
...
// ------------------- 1件分の入力/編集画面 -------------------
function JobDetail({ dateKey, job, onSave, onSaveAndAddPerson, onBack, companies, setCompanies, sitePresets, setSitePresets, ninkuPresets, setNinkuPresets, onSaveCombo, comboPresets, routes, setRoutes, entries }) {
  ...
  const buildJob = () => ({ company, site, ninku: Number(ninku), overtimeMode, overtimeMin, overtimeRatePerHour, overtimeAmount, expenses, transports, memo });
  const save = () => onSave(buildJob());
  const saveAndAddPerson = () => onSaveAndAddPerson(buildJob());
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#1C1F26", fontFamily: "'Zen Kaku Gothic New','Hiragino Sans',sans-serif" }}>
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "18px 16px 60px" }}>
        ...
        <button onClick={saveAndAddPerson} style={{ marginTop: 10, width: "100%", background: "#242832", border: "1px dashed #F5A623", borderRadius: 12, padding: "12px", color: "#F5A623", fontSize: 13, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, cursor: "pointer" }}>
          <Plus size={15} />保存して、同じ現場でもう1人分を入力する
        </button>
      </div>
...
// ------------------- 会社ごとの履歴画面 -------------------
function HistoryView({ entries, companies, onBack, onOpenDate }) {
  // entries全体から、会社ごとに「日付・現場名・金額」のリストを作る
  const byCompany = useMemo(() => {
    ...
    Object.entries(entries).forEach(([k, jobs]) => {
      (jobs || []).forEach((j) => {
        ...
        if (!map[c]) map[c] = [];
        map[c].push({ date: k, site: j.site || "", total: jobTotal(j) });
      });
    });
    Object.keys(map).forEach((c) => map[c].sort((a, b) => b.date.localeCompare(a.date))); // 新しい順
    return map;
  }, [entries]);

  const allCompanyNames = useMemo(() => {
    const set = new Set([...companies, ...Object.keys(byCompany)]);
    return Array.from(set).filter((c) => (byCompany[c] || []).length > 0);
  }, [companies, byCompany]);

  const [selected, setSelected] = useState(allCompanyNames[0] || null);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#1C1F26", fontFamily: "'Zen Kaku Gothic New','Hiragino Sans',sans-serif" }}>
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "18px 16px 60px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18 }}>
          <button onClick={onBack} style={iconBtnStyle}><ChevronLeft size={22} color="#F5A623" /></button>
          <div style={{ color: "#fff", fontSize: 18, fontWeight: 800 }}>会社ごとの履歴</div>
        </div>

        {allCompanyNames.length === 0 ? (
          <div style={{ textAlign: "center", padding: "30px 0", color: "#8A8F9C", fontSize: 13 }}>まだ入力がありません</div>
        ) : (
          <>
            <div style={{ color: "#8A8F9C", fontSize: 12, fontWeight: 700, marginBottom: 8 }}>会社を選ぶ</div>
            <ChipRow>
              {allCompanyNames.map((c) => (
                <Chip key={c} active={selected === c} onClick={() => setSelected(c)}>
                  {c}（{(byCompany[c] || []).length}件）
                </Chip>
              ))}
            </ChipRow>
            ...
            {selected && (
              <div style={{ marginTop: 20 }}>
                <div style={{ color: "#8A8F9C", fontSize: 12, fontWeight: 700, marginBottom: 8 }}>
                  {selected} の記録（{(byCompany[selected] || []).length}件）
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {(byCompany[selected] || []).map((r, i) => (
                    <button
                      key={i}
                    ...
                    >
                      <div>
                        ...
                        {r.site && <div style={{ color: "#6B7280", fontSize: 11, marginTop: 2 }}>{r.site}</div>}
                      </div>
      ...
      .forEach(([k, jobs]) => {
        (jobs || []).forEach((j) => {
          out.push({
            ...
            transport: transportsTotal(j.transports), transports: j.transports || [],
            miscExpense: expensesTotal(j.expenses), expenses: j.expenses || [],
            total: jobTotal(j),
          });
        });
      });
