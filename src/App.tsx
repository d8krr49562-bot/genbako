...
export default function InvoiceApp() {
  ...
  useEffect(() => {
    ...
    try {
      const payload = JSON.stringify({ entries, companies, ninkuPresets, comboPresets, routes, profile });
  ...
  }, [entries, companies, ninkuPresets, comboPresets, routes, profile, loading]);
...
function PdfPreview({ year, month, rows, grandTotal, companyLabel, profile, onBack }) {
  ...
  const [editableRows, setEditableRows] = useState(
    rows.map((r) => ({ ...r, dateLabel: `${month}月${r.day}日`, content: r.site ? `${r.company}　${r.site}` : r.company }))
  );
