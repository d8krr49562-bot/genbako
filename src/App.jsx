...
export default function InvoiceApp() {
  ...
  if (view === "invoice") {
    return <InvoiceView year={year} month={month} entries={entries} onBack={() => setView("calendar")} profile={profile} setProfile={setProfile} />;
  }
...
// ------------------- まとめて予定追加画面 -------------------
function BulkAddView({ initialYear, initialMonth, entries, comboPresets, onBack, onApply }) {
        ...
        {comboPresets.length === 0 ? (
          ...
          <>
            ...
            <div style={{ color: "#8A8F9C", fontSize: 12, fontWeight: 700, marginBottom: 8 }}>
              ② 日付を選ぶ（{selectedDates.length}件選択中）
            </div>
...
// ------------------- 請求内容確認画面 -------------------
function InvoiceView({ year, month, entries, onBack, profile, setProfile }) {
  const rows = useMemo(() => {
    const out = [];
